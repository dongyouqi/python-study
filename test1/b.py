...
def get_pic_list(html):
    ...
    pic_list = soup.find_all('li', class_='line')
    for i in pic_list:
        a_tag = i.find('a')
        link = a_tag.get('href')
        link = 'https://www.badazhou.com' + link

        text = i.find('h3').get_text().strip()
        text = link.split('/')[-1]
...
def main():
    ...
    while len(queue) > 0:
        ...
        while len(threads) < 5 and len(queue) > 0:   # 最大线程数设置为 5
            ...
            url = 'https://www.badazhou.com/trip/themeList/{}'.format(cur_page)
